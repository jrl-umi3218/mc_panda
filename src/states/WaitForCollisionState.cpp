...
bool WaitForCollisionState::run(mc_control::fsm::Controller & ctl_)
{
  ...
  if(sensorAvailable){
    ...
    for(int i=0; i<7; i++){
      ...
      {
        mc_rtc::log::info("WaitForCollisionState detected a joint-space collision: {}", joint_contactVector_thresholds_.transpose());
    ...
    cartesian_contactVector_ = ctl_.robot().device<mc_panda::PandaSensor>(sensorDeviceName).get_O_F_ext_hat_K();
    ...
    for(int i=0; i<6; i++){
      ...
      {
        mc_rtc::log::info("WaitForCollisionState detected a cartesian-space collision: {}", cartesian_contactVector_.transpose());
        mc_rtc::log::info("Completed WaitForCollisionState");
        collisionDetected = true;
        output("OK");
        return true;
  ...
  {
      mc_rtc::log::info("WaitForCollisionState detected a force().z() collision: {}", forceSensor.wrench().force().z());
      mc_rtc::log::info("Completed WaitForCollisionState");
      collisionDetected = true;
      output("OK");
      return true;
...
void WaitForCollisionState::addToLogger(mc_rtc::Logger & logger)
{
  ...
  logger.addLogEntry(logname + "tauexthatfilteredThresholds", 
    [this]() {
      return (Eigen::VectorXd) joint_contactVector_thresholds_; 
    }
  );
  logger.addLogEntry(logname + "OFexthatKThresholds", 
    [this]() {
      return (Eigen::VectorXd) cartesian_contactVector_thresholds_; 
    }
  );
